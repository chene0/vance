...
import { Button, Modal, TextInput, FileInput, Pagination, FloatingLabel, Alert } from "flowbite-react";
...
import { AddFolder, AdjustQuestionPriorityRating, CheckLambdaCompletion, CreateQuestion, CreateSet, CreateWorkerTest, DeleteAutogeneratedQuestionsByFileId, DeleteFolder, DeleteQuestionById, DeleteSet, EditQuestion, GetAllQuestionData, GetAutogeneratedQuestionData, GetQuestionData, GetQuestionDataById, ProcessFile } from "../lib/actions";
...
export function Container({ user }: { user: any }) {
    ...
    const [modalTryAgainLater, setModalTryAgainLater] = useState(false);
    ...
    const [modalAutogenQuestionDeletionState, setModalAutogenQuestionDeletionState] = useState(false);
    const selectedFile = useAppSelector(selectWorkspace);
    const dispatch = useAppDispatch();

    // Variables for toasts
    ...
    const AdjustQuestionPriorityRatingAndRefresh = async (deltaPriorityRating: number) => {
        await AdjustQuestionPriorityRating(selectedQuestion[0], deltaPriorityRating);
        setSelectedQuestion([]);
        const allQuestions = await GetAllQuestionData(selectedFile.raw);
        setQuestionQueueRender(RenderQuestionQueue(await allQuestions));
    ...
    const ProcessFileThenRender = async () => {
        console.log("INITIATE PROCESSING OF FILE");
        ProcessFile(selectedFile.raw, numPages as number).then(async () => {
            console.log("INITIATE RENDERING OF QUESTION BOXES");
    ...
    return (
        <div className="bg-white min-h-screen">
                    ...
                    <div className="sticky top-0 divide-y divide-solid">
                        ...
                        <div className="mt-4 h-full divide-y divide-solid">
                                                ...
                                                {selectedQuestion.length > 1 ? <div></div>
                                                    : <div>
                                                        ...
                                                        <Button.Group className="my-4">
                                                            <Button onClick={async () => {
                                                                await AdjustQuestionPriorityRatingAndRefresh(-2);
                                                            }}>Clueless</Button>
                                                            <Button onClick={async () => {
                                                                await AdjustQuestionPriorityRatingAndRefresh(-1);
                                                        ...
                                                        <div>
                                                            <form onSubmit={async (event) => {
                                                                ...
                                                                <TextInput name="notes" placeholder={selectedQuestion[0].notes} />
                            ...
                            <div className="">
                                ...
                                <form action={async () => {
                                    const isLambdaCompleted = await CheckLambdaCompletion(selectedFile.raw, numPages as number);
                                    if (!isLambdaCompleted) {
                                        setModalTryAgainLater(true);
                                        return;
                                    }
                                ...
                                {/* DELETE AUTOGEN QUESTIONS */}
                                <Button
                                    className="mx-auto my-2"
                                    onClick={() => {
                                        setModalAutogenQuestionDeletionState(true);
                                    }}
                                >
                                    Delete autogenerated questions
                                </Button>
                            </div>
            ...
            <div> {/* MODALS */}
                {/* MODAL FOR TRY AGAIN LATER */}
                <Modal show={modalTryAgainLater} onClose={() => setModalTryAgainLater(false)} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            ...
                            <div className="flex justify-center gap-4">
                                <Button onClick={() => setModalTryAgainLater(false)}>
                                    I understand
                                </Button>
                            </div>
                        </div>
                ...
                <Modal show={modalSetProcessState} onClose={() => setModalSetProcessState(false)} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-gray-400 dark:text-gray-200" />
                            <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                Attention: Proceeding will result in the deletion of your old <b>autogenerated</b> questions
                            </h3>
                            <div className="flex justify-center gap-4">
                                <Button color="gray" onClick={async (event: any) => {
                                    event.preventDefault();
                                ...
                                <Button onClick={() => setModalSetProcessState(false)}>
                                    Cancel
                                </Button>
                            </div>
                        </div>
                ...
                <Modal show={modalSetDeletionState.open} onClose={() => dispatch(setModalSetDeletionState(''))} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-gray-400 dark:text-gray-200" />
                            ...
                            <div className="flex justify-center gap-4">
                                ...
                                <Button onClick={() => dispatch(setModalSetDeletionState(''))}>
                                    No, cancel
                                </Button>
                            </div>
                        </div>
                ...
                <Modal show={modalFolderDeletionState.open} onClose={() => dispatch(setModalFolderDeletionState(''))} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-gray-400 dark:text-gray-200" />
                            ...
                            <div className="flex justify-center gap-4">
                                ...
                                <Button onClick={() => dispatch(setModalFolderDeletionState(''))}>
                                    No, cancel
                                </Button>
                            </div>
                        </div>
                ...
                <Modal show={modalQuestionDeletionState.open} onClose={() => dispatch(setModalQuestionDeletionState([]))} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-gray-400 dark:text-gray-200" />
                            ...
                            <div className="flex justify-center gap-4">
                                ...
                                <Button onClick={() => dispatch(setModalQuestionDeletionState([]))}>
                                    No, cancel
                                </Button>
                            </div>
                        </div>
                ...
                {/* MODAL FOR DELETING AUTOGEN QUESTIONS */}
                <Modal show={modalAutogenQuestionDeletionState} onClose={() => setModalAutogenQuestionDeletionState(false)} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <div className="text-center">
                            <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-gray-400 dark:text-gray-200" />
                            <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                Are you sure you want to delete all autogenerated questions?
                            </h3>
                            <div className="flex justify-center gap-4">
                                <Button color="gray" onClick={async (event: any) => {
                                    event.preventDefault();
                                    // Call backend to delete all autogenerated questions
                                    await DeleteAutogeneratedQuestionsByFileId(selectedFile.raw);
                                    // REFRESH QUESTION BOXES AND QUEUE
                                    const currentQuestions = await GetQuestionData(selectedFile.raw, pageNumber - 1);
                                    setQuestionBoxRender(RenderQuestionBoxes(await currentQuestions));
                                    const allQuestions = await GetAllQuestionData(selectedFile.raw);
                                    setQuestionQueueRender(RenderQuestionQueue(await allQuestions));
                                    // Reset modal state
                                    setModalAutogenQuestionDeletionState(false);
                                }}>
                                    {"Yes, I'm sure"}
                                </Button>
                                <Button onClick={() => setModalAutogenQuestionDeletionState(false)}>
                                    No, cancel
                                </Button>
                            </div>
                        </div>
