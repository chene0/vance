...
export const CheckLambdaCompletion = async (file: string, numPages: number) => {
  const key = `${file}/${numPages-1}.jpg`;
...
export const CreateWorkerTest = async () => {
  fs.copySync(path.resolve(__dirname, './tesseract-core-simd.wasm'), path.resolve(__dirname, 'node_modules/tesseract.js-core'));
...
export const LambdaProcessFile = async (file: string, numPages: number) => {
...
export const ProcessFile = async (file: string, numPages: number) => {
  ...
  for(let i = 0; i < numPages; i++){
    ...
    await (async () => {
      const worker = await Tesseract.createWorker('eng', 1, {workerPath: "./node_modules/tesseract.js/src/worker-script/node/index.js"});
